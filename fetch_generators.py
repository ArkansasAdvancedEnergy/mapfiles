...
STATE     = "AR"
MAX_ROWS  = 5000   # EIA max per request
...
def deduplicate(records):
    ...
    Unique key: plantCode + generatorId (both present in EIA data).
...
def to_geojson(records):
    ...
    for r in records:
        ...
        # Skip records with null/zero coordinates
        ...
        props = {
            "plantName":          r.get("plantName") or r.get("plant-name") or "—",
            "entityName":         r.get("entityName") or r.get("entity-name") or "—",
...
def main():
    ...
    # Print a quick status breakdown
    statuses = {}
    for feat in geojson["features"]:
        s = feat["properties"]["status"].split(" (")[0]  # group by base status
